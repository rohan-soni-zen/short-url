...
const URLForm = ({ className, setRecents, ...props }: URLFormProps) => {
	...
	async function onSubmit(data: z.infer<typeof FormSchema>) {
		toast.promise(shortenUrl({ longURL: data.url, alias: data.alias }), {
			loading: "Shortening URL...",
			success: (res: { data: RecentEntry }) => {
				setRecent(res.data);
				setRecents((r: RecentEntry[]) => [...r.slice(), res.data]);
				return "URL shortened successfully!";
			},
			error: (err: any) => {
				return err.response?.data?.error || "Failed to shorten URL";
			},
		});
	}
								...
								<FormField
									...
									render={({ field }: { field: any }) => (
										<FormItem>
											...
											<FormDescription>
												Create a custom short URL. Leave
